...
$sql_check="select * from number where game_id=$game_id and value=$number and check_state=2"  ;
...
$sql="update number set check_state=2 where game_id=$game_id and value=$number" ;
...
{
	header("Location: ../check_bingo.php?game_id=".$game_id."&new_number=".$number);
	mysqli_close($con);
}
else
{
	header("Location: ../check_bingo.php?game_id=".$game_id."&new_number=".$number."&num_found=1");	
	mysqli_close($con);
}
